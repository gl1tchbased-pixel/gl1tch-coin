...
/**
 * GL1TCH — "THE SCANNER JUST LEVELED UP" (~24s, 9:16). Original feature-showcase:
 * the V3 differentiators (AI verdict · degen intel · verified blue-chips · shareable
 * badge) as animated cards. No reused clips. Brand palette.
 */
...
const Hook: React.FC<{ dur: number }> = ({ dur }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: seg(f, 0, dur) }}>
      <div style={{ position: "absolute", top: 250, left: 0, right: 0, textAlign: "center", fontFamily: "monospace", fontSize: 28, letterSpacing: 7, color: GREEN, opacity: rise(f, 6, 20) }}>GL1TCH // SCANNER v3</div>
      <div style={{ position: "absolute", top: 720, left: 0, right: 0, textAlign: "center", padding: "0 60px" }}>
        <Chroma text={"THE SCANNER\nLEVELED UP."} size={92} split={split} />
      </div>
      <div style={{ position: "absolute", top: 1020, left: 0, right: 0, textAlign: "center", padding: "0 80px", fontFamily: "monospace", fontSize: 32, color: "rgba(245,247,248,0.85)", opacity: rise(f, 40, 60) }}>
...
const Cta: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: INK, opacity: rise(f, 0, 14) }}>
      ...
      <div style={{ position: "absolute", top: 720, left: 0, right: 0, textAlign: "center", padding: "0 56px" }}>
        <Chroma text={"SCAN BEFORE\nYOU APE."} size={96} split={split} />
      </div>
      <div style={{ position: "absolute", top: 1040, left: 0, right: 0, textAlign: "center", fontSize: 44, fontWeight: 800, color: GREEN }}>coin-three-mu.vercel.app/scan</div>
      ...
      <div style={{ position: "absolute", bottom: 150, left: 0, right: 0, textAlign: "center", fontFamily: "monospace", fontSize: 24, letterSpacing: 3, color: GREEN, opacity: 0.85 }}>free · non-custodial · any chain</div>
    </AbsoluteFill>
  );
};

const H = 140, FD = 145, CTA = 150;
const f1 = H - 12, f2 = f1 + FD - 12, f3 = f2 + FD - 12, f4 = f3 + FD - 12, cta = f4 + FD - 12;
const TOTAL = cta + CTA;

export const ScannerUpgraded: React.FC = () => (
  <AbsoluteFill style={{ backgroundColor: INK, overflow: "hidden" }}>
    <Sequence durationInFrames={H}><Hook dur={H} /></Sequence>
    <Sequence from={f1} durationInFrames={FD}><Feature dur={FD} icon="🤖" tag="AI VERDICT" title={"IT TELLS YOU\nSTRAIGHT."} accent={GREEN} lines={["reads the chain", "then writes the verdict", "in its own words"]} /></Sequence>
    <Sequence from={f2} durationInFrames={FD}><Feature dur={FD} icon="🐋" tag="DEGEN INTEL" title={"SEES THE\nWHALES."} accent={"#FFD14F"} lines={["insiders & bundled wallets", "holder spread · snipers", "fake liquidity exposed"]} /></Sequence>
    <Sequence from={f3} durationInFrames={FD}><Feature dur={FD} icon="🛡" tag="VERIFIED" title={"REAL, NOT\nA CLONE."} accent={GREEN} lines={["canonical blue-chips flagged", "impostor tokens caught", "correct price, every chain"]} /></Sequence>
    <Sequence from={f4} durationInFrames={FD}><Feature dur={FD} icon="📡" tag="SHARE IT" title={"FLEX THE\nVERDICT."} accent={PURPLE} lines={["shareable scan card", "embeddable badge", "scanned by GL1TCH"]} /></Sequence>
...
export const SCANNER_UPGRADED_FRAMES = TOTAL;
