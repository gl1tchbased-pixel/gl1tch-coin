...
/**
 * GL1TCH — "THE SCANNER GREW A MEMORY" (~20s, 9:16). Premium-features announcement:
 * Signal Graph (cross-scan deployer memory) · Proof-of-Signal (unfakeable reputation) ·
 * Security & Transparency. Motion-graphics only, brand palette, no reused AI clips.
 */
...
const Hook: React.FC<{ dur: number }> = ({ dur }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity: seg(f, 0, dur) }}>
      <div style={{ position: "absolute", top: 250, left: 0, right: 0, textAlign: "center", fontFamily: "monospace", fontSize: 28, letterSpacing: 7, color: GREEN, opacity: rise(f, 6, 20) }}>GL1TCH // SIGNAL GRAPH</div>
      <div style={{ position: "absolute", top: 700, left: 0, right: 0, textAlign: "center", padding: "0 56px" }}>
        <Chroma text={"THE SCANNER\nGREW A MEMORY."} size={86} split={split} />
      </div>
      <div style={{ position: "absolute", top: 1040, left: 0, right: 0, textAlign: "center", padding: "0 80px", fontFamily: "monospace", fontSize: 31, color: "rgba(245,247,248,0.85)", opacity: rise(f, 40, 60) }}>
        others forget. GL1TCH remembers.
      </div>
    </AbsoluteFill>
  );
};
...
const Cta: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: INK, opacity: rise(f, 0, 14) }}>
      ...
      <div style={{ position: "absolute", top: 700, left: 0, right: 0, textAlign: "center", padding: "0 56px" }}>
        <Chroma text={"THE SIGNAL\nREMEMBERS."} size={92} split={split} />
      </div>
...
export const SignalGraphVideo: React.FC = () => (
  <AbsoluteFill style={{ backgroundColor: INK, overflow: "hidden" }}>
    <Sequence durationInFrames={H}><Hook dur={H} /></Sequence>
    <Sequence from={f1} durationInFrames={FD}><Feature dur={FD} icon="🧠" tag="SIGNAL GRAPH" title={"IT\nREMEMBERS."} accent={GREEN} lines={["every deployer, every scan", "2+ rugs = serial deployer", "flagged before you buy"]} /></Sequence>
    <Sequence from={f2} durationInFrames={FD}><Feature dur={FD} icon="📡" tag="PROOF-OF-SIGNAL" title={"REP YOU\nCAN'T FAKE."} accent={AMBER} lines={["verified sustained holding", "7-day avg · no flash-buys", "climb to Beacon Prime"]} /></Sequence>
    <Sequence from={f3} durationInFrames={FD}><Feature dur={FD} icon="🛡" tag="SECURITY" title={"SAFE\nITSELF."} accent={PURPLE} lines={["never a fund signature", "CSP · secret + dep scans", "public threat model"]} /></Sequence>
...
export const SIGNAL_GRAPH_FRAMES = TOTAL;
